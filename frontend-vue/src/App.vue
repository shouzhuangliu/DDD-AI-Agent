...
function capVersionActions(kind, status) {
  ...
  if (kind === 'mcp') {
    ...
    if (s === 'DISCOVERED') return ['discover', 'scan'];
...
function capActionLabel(kind, action, status) {
  const normalizedStatus = String(status || '').toUpperCase();
  if (kind === 'mcp') {
    return {
      connect: '连通性测试',
      discover: normalizedStatus === 'DISCOVERED' ? '重新探测工具' : '探测工具',
      scan: '安全扫描',
      sandbox: '沙箱测试',
      submit: '提交审核',
      reviewTest: '测试审核通过',
      reviewSecurity: '安全审核通过',
      release: '发布 MCP',
    }[action] || action;
  }
  return {
        ...
        <div class="page">
          ...
          <template v-if="tab === 'dashboard'">
            ...
            <section class="panel section-gap">
              ...
              <div class="memory-recall-hint">???? Agent ?????/??? Case ???????????????????????</div>
              <div class="memory-recall-grid">
                <div v-if="!memoryRecallResults.length" class="empty">?????????????????????????</div>
                <div v-for="item in memoryRecallResults" :key="`${item.sourceType}-${item.sourceId}-${item.summary}`" class="memory-recall-card">
                  <div class="item-row">
                    <div>
                      <div class="memory-recall-title">{{ item.kind || item.sourceType || '????' }}</div>
                      <small class="muted">{{ item.sourceType || '-' }} ? {{ item.sourceId || '-' }}</small>
                    </div>
                    <span class="pill">{{ item.score ?? '-' }}</span>
                  </div>
                  <div class="memory-recall-body">{{ item.summary || item.memory || item.content || '-' }}</div>
                  <div class="memory-recall-meta">
                    <span v-if="item.caseId" class="pill">Case {{ item.caseId }}</span>
                    <span v-if="item.sessionId" class="pill">?? {{ item.sessionId }}</span>
                    <span v-if="item.feedbackId" class="pill">?? {{ item.feedbackId }}</span>
                  </div>
                </div>
              </div>
            </section>
                ...
                <div class="list">
                  ...
                  <div v-for="version in mcpVersions" :key="pick(version, 'id', 'ID')" class="item">
                    ...
                    <div class="actions" style="margin-top:10px;gap:6px">
                      ...
                      >
                         {{ capActionLabel('mcp', action, pick(version, 'status', 'STATUS')) }}
                      </button>
                    </div>
                  </div>
                </div>
              ...
              <section class="panel">
                <template v-if="selectedLocalSkill">
                  ...
                  <div class="muted" style="font-size:12px;margin-bottom:10px">
                    当前读取全局本地 Skill 源文件；Agent 对话运行时只会注入已绑定的 Skill。
                  </div>
                ...
                <div class="list">
                  ...
                  <div v-for="version in skillVersions" :key="pick(version, 'id', 'ID')" class="item">
                    ...
                    <div class="actions" style="margin-top:10px;gap:6px">
                      ...
                      >
                         {{ capActionLabel('skill', action, pick(version, 'status', 'STATUS')) }}
                      </button>
                     </div>
                   </div>
                 </div>
           ...
           <div class="field-grid">
            ...
            <div class="field">
              ...
              <select v-model="modal.form.transportType">
                <option value="stdio">STDIO（本地脚本）</option>
            ...
            <div class="field field-full">
              <label>端点配置 JSON</label>
              <textarea v-model="modal.form.endpointConfig" rows="8" placeholder='STDIO: {"command":"python","args":["server.py"]}；HTTP: {"baseUri":"http://localhost:3000"}'></textarea>
            </div>
          </div>
          <div class="field-actions">
            <button class="btn" @click="closeModal">取消</button>

...
public class AutoAgentTest {
    ...
    public void init() {
        ...
        planningChatClient = ChatClient.builder(chatModel)
                .defaultSystem("""
                    ...
                    - 🚨 特殊要求：如果用户的需求涉及“创建文档”、“生成文件”、“保存到本地”等操作，你必须将“调用文件操作工具进行保存落盘”作为一个独立的步骤，明确写进下方的任务规划中！
                    ...
                    ## 输出格式
                    请严格按以下格式输出任务计划（绝不允许破坏此模板结构）：
                    
                    **思考过程：**
                    [在这里详细写下你对用户需求的分析、拆解逻辑，以及是否需要调用文件工具的判断]
    ...
    public void test_complete_auto_agent_workflow() {
        String userRequest = "帮我撰写一个关于Spring AI框架的技术文档（包括核心概念、使用示例和最佳实践）。撰写完成后，请务必调用文件操作工具，将该文档保存到D:\\\\javacode";
        ...
        String executionContext = String.format("""
                你现在是一个没有任何感情的自动文档生成器。
                
                        【原始需求】：%s
                        【规划清单】：%s
                
                        🚨【强制指令】：
                        1. 必须一次性、连贯地完成以上规划的所有步骤！
                        2. 直接输出最终的完整 Markdown 技术文档（包含详细解释、代码示例、最佳实践等）。
                        3. 绝对禁止分步骤汇报！
                        4. 绝对禁止在结尾问我"是否继续"或"需要修改吗"等任何客套话！
    ...
    public void test_dynamic_multi_step_execution() {
        ...
        userInput = "搜索 springboot 相关知识，生成4个主要内容章节。每个章节要包括课程内容和配套示例代码。并发对应章节创建对md文档，方便小白伙伴学习。撰写完成后，请务必调用文件操作工具，将该文档保存到D:\\\\javacode";
    ...
    public McpSyncClient stdioMcpClient() {
        ...
        var stdioParams = ServerParameters.builder("\"D:\\deeping study\\node\\npx.cmd\"")
                .args("-y", "@modelcontextprotocol/server-filesystem@2025.3.28", "D:\\javacode", "D:\\javacode")
                .build();

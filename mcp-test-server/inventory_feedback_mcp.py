"""库存业务 Feedback MCP（本地 STDIO 演示服务）。

协议：JSON-RPC 2.0，一行一个 JSON 请求/响应。服务不向 stdout 输出日志，
因此可以直接作为 MCP STDIO 子进程被 AI Agent 调用。
"""
...
DATA_DIR = Path(os.environ.get("INVENTORY_FEEDBACK_MCP_DATA_DIR", Path(__file__).parent / "data"))
DATA_DIR.mkdir(parents=True, exist_ok=True)
TRIAGE_FILE = DATA_DIR / "inventory_triage.jsonl"
...
TODAY_FEEDBACK = [
    {
        "feedbackId": f"fb-inv-{TODAY.replace('-', '')}-001",
        "source": "USER",
        "service": "product-availability",
        "summary": "DDR5 内存详情页显示可下单，但支付后被通知缺货",
        "content": "用户反馈 DDR5 32G 5600MHz 内存详情页显示有货并可正常下单，支付后客服通知实际缺货，希望尽快补货并排查库存展示问题。",
    ...
    {
        "feedbackId": f"fb-inv-{TODAY.replace('-', '')}-002",
        "source": "OPS",
        "service": "inventory-core",
        "summary": "RTX 5060 缓存库存与后台库存不一致",
        "content": "运营巡检发现 RTX 5060 频道页显示剩余 12 件，但后台库存中心实际只有 2 件，怀疑缓存未及时刷新。",
    ...
    {
        "feedbackId": f"fb-inv-{TODAY.replace('-', '')}-003",
        "source": "MONITOR",
        "service": "stock-reservation",
        "summary": "锁库存释放延迟导致部分商品短时间无法再次下单",
        "content": "监控显示锁库存释放任务出现积压，取消订单后的库存没有及时回补，影响用户再次下单。",
    ...
    {
        "feedbackId": f"fb-inv-{TODAY.replace('-', '')}-004",
        "source": "USER",
        "service": "replenishment-sync",
        "summary": "用户希望补货 2TB SSD，但暂未提供 SKU",
        "content": "用户表示 2TB SSD 长期缺货，希望平台补货，但没有提供品牌、型号或商品 ID。",
...
TOOLS = [
    {
        ...
        "description": "读取今日库存业务 Feedback，支持按来源、服务域和数量过滤。",
        "inputSchema": {
            "type": "object",
            "properties": {
                "limit": {"type": "integer", "description": "最多返回条数，默认全部，最大 50"},
    ...
    },
    {
        ...
        "description": "写入一条本地分诊记录，演示 Feedback 到 Case 的评测闭环。",
        "inputSchema": {
            "type": "object",
            "properties": {
                ...
                "operator": {"type": "string", "description": "执行分诊的 Agent 或操作人"},
...
def _read_triage_rows() -> list[dict[str, Any]]:
    if not TRIAGE_FILE.exists():
        return []
    return [json.loads(line) for line in TRIAGE_FILE.read_text(encoding="utf-8").splitlines() if line.strip()]
...
def call_tool(name: str, arguments: dict[str, Any] | None) -> Any:
    ...
    if name == "get_today_feedback":
        ...
        rows = [item for item in TODAY_FEEDBACK if (not source or item["source"] == source)
                and (not service or item["service"].lower() == service)]
        return rows[:limit]
    if name == "get_feedback_detail":
        detail = _feedback_by_id(_required(arguments, "feedbackId"))
        if not detail:
    ...
    if name == "search_feedback_by_keyword":
        ...
        limit = max(1, min(int(arguments.get("limit", len(TODAY_FEEDBACK))), 50))
        rows = [item for item in TODAY_FEEDBACK if query in json.dumps(item, ensure_ascii=False).lower()]
        return rows[:limit]

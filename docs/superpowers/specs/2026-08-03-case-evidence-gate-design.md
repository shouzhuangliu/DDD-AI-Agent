...
当前代码已经具备“会话空闲 60 秒后入队”和“5 分钟会话去重窗口”，本设计保留这两个基础设施，但补充业务语义阈值，避免每个助手回复都触发 Case 总结：

- **首次 Case 评测**：默认至少累计 2 个有业务内容的用户/运维回合，并且会话空闲 60 秒；明确的人工负反馈或高危 MCP 业务结果可以提前进入评测，但仍必须经过证据门禁；
- **后续重新评测**：上次评测后至少新增 2 条有业务内容的用户/运维证据，或出现明确人工 Feedback、Skill 规则变化、绑定 MCP 新结果时才重新评测；没有新增证据时直接跳过；
- **Case 摘要生成**：只在首次通过 `CANDIDATE_CASE`、人工审核变更、或新增证据达到重新评测阈值时生成/更新。`FEEDBACK_CAPTURED` 和 `NEED_MORE_INFO` 不生成 Case 摘要，只展示“缺失信息”；
- **同一 Case 的频率控制**：同一 `caseId + policyVersion + evidenceFingerprint` 幂等，重复任务只追加未见过的证据，不重复生成摘要；
- **短期记忆摘要**：与 Case 评测独立。保留最近 24 条消息，至少累计 4 个新的有业务内容用户回合且超过 8,000 token 才滚动总结；若超过 16,000 token 作为硬上限立即总结。单条“1/OK/继续”不会推进摘要版本；
- **长期记忆写入**：只接受通过短期摘要质量门禁的稳定事实，Case 候选和运行故障不会自动写入长期记忆。

因此，仪表盘展示的 Case 数量和摘要数量都由“新证据窗口”驱动，而不是由聊天消息数量直接驱动。
...
- `CaseAnalysisCadencePolicy`：根据用户回合数、空闲时间、新增证据指纹和显式 Feedback 决定是否需要重新评测；
...
- 两条普通助手回复但没有新增用户/运维证据：不生成新的 Case 摘要；
- 新增两条业务证据后才触发一次重新评测；同一证据指纹重复执行不增加摘要版本；
- 短期记忆在 4 个新业务用户回合或 token 硬上限时触发，单字消息不会触发。

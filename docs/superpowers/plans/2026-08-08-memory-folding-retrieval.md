...
- [x] **Step 3: Add the metadata fields and DAO contract**
...
- [x] **Step 3: Implement the deterministic folding stages**
...
- [x] **Step 4: Verify message-pair invariants**
...
- [x] **Step 2: Run the test and confirm current failure**
...
- [x] **Step 3: Return full history metadata from `ChatMessageRecorderDb`**
...
- [x] **Step 4: Replace duplicated message conversion**
...
- [x] **Step 1: Write security and recovery tests**
...
- [x] **Step 2: Implement session-scoped lookup and bounded response**
...
- [x] **Step 3: Add observability events**
...
- [x] **Step 1: Write the persistence tests**
...
- [x] **Step 2: Verify configuration defaults**
...
- [x] **Step 3: Make summary state update atomic**
...
- [x] **Step 4: Run memory tests and commit**
...
- [x] **Step 1: Add the contract scenario**
...
- [x] **Step 2: Run module regression**
...
- [x] **Step 3: Document the data flow**
...
- [x] **Step 4: Commit the final verification**
...
- [x] 数据库原始消息未被折叠逻辑修改。
- [x] 每次发给 LLM 的消息都通过 `sanitize`，不存在孤立 `tool` 或未闭合 `tool_calls`。
- [x] 折叠后的工具结果包含稳定的 `tool_call_id` 指针。
- [x] 取回工具按 `sessionId + toolCallId` 隔离数据。
- [x] 取回只读取历史原文，不重新执行原工具。
- [x] 滚动摘要按覆盖游标增量折叠，服务重启后可恢复。
- [x] 折叠、取回命中、取回失败和截断均进入运行轨迹。
- [x] 全量 Maven 测试通过后，才在简历中描述为完整闭环。
